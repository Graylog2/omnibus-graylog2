add_command 'set-listen-address', 'Set the IP address on which Graylog\'s API/UI is listening on', 2 do |cmd_name|
  ...
  ::OptionParser.new do |opts|
    opts.on("--service NAME", String, "Service to set web|rest|transport") do |value|
      options[:service] = value
    end
    opts.on("--address ADDRESS", String, "Listen URI, like http://127.0.0.1:12900") do |value|
  ...
  if options[:service] && options[:address]
    ...
    puts "Usage: #{cmd_name} --service <web|rest|transport> --address <http://someaddress:port>"
    exit
  end
